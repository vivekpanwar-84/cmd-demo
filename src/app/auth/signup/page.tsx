...
export default function SignupPage() {
  ...
  const handleSignup = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      document.cookie =
        "auth_token=mock_token; path=/; max-age=86400; SameSite=Strict";
      setLoading(false);
      router.push("/");
      router.refresh();
    }, 1500);
        ...
        <CardContent>
          <form onSubmit={handleSignup} className="space-y-4">
            ...
            <Button 
              type="submit" 
              // Button Background
              className="w-full bg-[#ff9966] hover:bg-[#ff9966]/90 text-white shadow-md hover:shadow-lg transition-all duration-300 ease-in-out transform hover:-translate-y-0.5 mt-2" 
              disabled={loading}
            >
              {loading ? (
                <>
